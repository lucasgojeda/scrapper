...
import {
  scrapperMLService,
  scrapperProductPageMLService,
} from "../services/scrapperML.services.js";
...
export const scrapperMLController = async (req, res) => {
  ...
  const results = await scrapperProductPageMLService(data);
  ...
  // /** Create excel file */
  await createExcelFile(results, name);
  // if (!url) {
